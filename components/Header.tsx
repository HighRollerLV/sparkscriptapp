...
export const Header: React.FC<HeaderProps> = ({ navigate, language, onLanguageChange, t }) => {
  return (
    <header className="bg-[#14171A]/80 backdrop-blur-sm border-b border-[#22272B] sticky top-0 z-10">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        <button onClick={() => navigate('about')} className="flex items-center gap-3" aria-label={t.aria_label_homepage}>
        ...
        <nav className="flex items-center gap-4">
            <button onClick={() => navigate('generator')} className="text-gray-300 hover:text-white transition-colors font-semibold">
                {t.promptBuilder}
            </button>
